...
const char* device_name = IOT_CONFIG_DEVICE_ID;


//Your Domain name with URL path or IP address with path
String serverName = "http://iot-project-15989.azurewebsites.net/api/push_values";
String homeAssistantServer = "http://homeassistant.local:8123/api/webhook/";
...
static unsigned long next_push_value_time = 0;
...
static void sendValues()
{
  ...
  if(WiFi.status()== WL_CONNECTED){
    PostRequest(temp_event.temperature, pressure_event.pressure);
  }
  else {
...
int PostRequest(double temperature, double pressure)
{
  WiFiClient client;
  HTTPClient http;

  String params = String("?User=") + device_name + 
  "&TEMP=" + temperature +
  "&PRES=" + pressure +
  "&HUMID=" + 0.0;
...
int triggerWebhook(char* webhook)
{
  WiFiClient client;
  HTTPClient http;

  String serverPath = homeAssistantServer + webhook;
...
{
  char* in_message = (char*)malloc((length + 1) * sizeof(char));
...
void loop()
{
  ...
  if (millis() > next_push_value_time)
  {
    sendValues();

    next_push_value_time = millis() + PUSH_VALUE_FREQUECNY;
  }
